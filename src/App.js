...
class App extends Component {
  ...
  render() {
   ...
   <div className="currency-converter">
      <CurrencyInput symbol={this.state.from} selectSymbol={sym => this.setState({from: sym})} setAmount={amt => this.setAmount(amt, "from")} amount={this.computeResult("from")} />
      <CurrencyInput symbol={this.state.to} selectSymbol={sym => this.setState({to: sym})} setAmount={amt => this.setAmount(amt, "from")} amount={this.computeResult("to")} />
    </div>
    )
  }
}
